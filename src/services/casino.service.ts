...
class CasinoService {
  ...
  getLiveCasinoData(slug: any) {
    return api.get(
      `${
        import.meta.env.VITE_APP_CASINO_SOCKET_URL
      }api/get-casino-market/${slug}`
    );
  }

  getResultByMid(type: string, mid: string) {
    return api.get(`html-cards/${type}/${mid}/?ismobile=${isMobile}`);
  }

  getResultList(
    filterdata: { matchid: string; startDate: string; roundId: string },
    page: number
  ) {
    return api.get(
