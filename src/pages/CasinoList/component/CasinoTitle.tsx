...
const CasinoTitle = (props: any) => {
  ...
  const refAndarBahar2 = useRef<any>(null);
  const refAndarBahar2B = useRef<any>(null);
  const refAndarBahar = useRef<any>(null);
  const refAndarBaharB = useRef<any>(null);
  ...
  useEffect(() => {
    ...
    if (andarData?.[1] && refAndarBahar2.current) {
      refAndarBahar2.current.swiper.slideTo(andarData?.[1].length - 1);
    }

    if (andarData?.[0] && refAndarBahar2B.current) {
      refAndarBahar2B.current.swiper.slideTo(andarData?.[0].length - 1);
    }
  }, [lastResult]);
  ...
  useEffect(() => {
    ...
    if (andarData && refAndarBahar.current) {
      refAndarBahar.current.swiper.slideTo(andarData.length - 1);
    }

    if (baharData && refAndarBaharB.current) {
      refAndarBaharB.current.swiper.slideTo(baharData.length - 1);
    }
      ...
      {lastResult && (lastResult.match_id || lastResult.id) && (
        <div className="video-overlaybox">
          ...
          {(getCurrentMatch.slug == 'dt20' ||
            ...
            dt20()}
          {(getCurrentMatch.slug == 'onedaypoker' ||
            getCurrentMatch.slug == 'onedaypoker20') &&
            poker()}

          {getCurrentMatch.slug == 'Tp1Day' &&
            getCurrentMatch?.match_id > 0 &&
            onedayteen()}
          {((!isMobile && getCurrentMatch.slug == 'dtl20') ||
            getCurrentMatch.slug == 'war') &&
            dtl20()}
