...
const Cmeter20 = (props: any) => {
  ...
  const onBet = (isBack = false, item: any) => {
    ...
    if (userState.user.role === RoleType.user) {
      ...
      if (
        parseFloat(oddsVal.toString()) <= 0 ||
        item.gstatus === 'SUSPENDED' ||
        item.gstatus === '0' ||
        (userBets &&
          userBets[0] &&
          userBets[0].selectionName &&
          userBets[0].selectionName != item.nat)
      )
        return;
      dispatch(
        betPopup({
  ...
  const lowCard = () => {
    return (
      <div className="text-center">
        ...
        {betValues.betData.selectionName == 'Low' ||
        (userBets &&
          userBets[0] &&
          userBets[0].selectionName &&
          userBets[0].selectionName == 'Low') ? (
          <img
            src={`https://dzm0kbaskt4pv.cloudfront.net/v11/static/front/img/andar_bahar/9meter.jpg`}
          />
        ) : (
          <img src="https://dzm0kbaskt4pv.cloudfront.net/v11/static/front/img/andar_bahar/9.jpg" />
        )}
      </div>
    );
  };
  const highCard = () => {
    return (
      <div className="text-center">
        {betValues.betData.selectionName == 'High' ||
        (userBets &&
          userBets[0] &&
          userBets[0].selectionName &&
          userBets[0].selectionName == 'High') ? (
          <img
            src={`https://dzm0kbaskt4pv.cloudfront.net/v11/static/front/img/andar_bahar/10meter.jpg`}
          />
        ) : (
          <img src="https://dzm0kbaskt4pv.cloudfront.net/v11/static/front/img/andar_bahar/10.jpg" />
        )}
    ...
    return liveMatchData?.defaultMarkets?.map((ItemData: any, key: number) => {
      ...
      const clsstatus =
        ...
        (userBets &&
          userBets[0] &&
          userBets[0].selectionName &&
          userBets[0].selectionName != ItemNew.RunnerName)
          ? 'suspended'
          : '';
      return (
        <div
          className={
            !isMobile
              ? `col-lg-6 col-12 ${key == 0 ? 'plr-0' : 'pr-0'}`
              : 'col-lg-6 col-12'
          }
          key={key}
        >
          <button
            className={`btn-theme ${clsstatus}`}
            onClick={() => onBet(true, Item)}
            style={{ minHeight: '135px' }}
          >
            <div className="cmeter-title">
              {((betValues.betData.selectionName == 'High' &&
                ItemNew.RunnerName == 'Low') ||
                (userBets?.[0]?.selectionName == 'High' &&
                  ItemNew.RunnerName == 'Low')) && (
                <img
                  key={key}
                  src={`https://dzm0kbaskt4pv.cloudfront.net/v11/static/front/img/cards/10HH.png`}
                  className={'mr-1'}
                />
              )}
              <h4 className="d-inline-block" style={{ minHeight: '53px' }}>
                <b>{ItemNew.RunnerName}</b>
              </h4>
              {((betValues.betData.selectionName == 'Low' &&
                ItemNew.RunnerName == 'High') ||
                (userBets?.[0]?.selectionName == 'Low' &&
                  ItemNew.RunnerName == 'High')) && (
                <img
                  key={key}
                  src={`https://dzm0kbaskt4pv.cloudfront.net/v11/static/front/img/cards/9HH.png`}
                  className={'ml-1'}
                />
              )}
            </div>
  ...
  const layoutCard = () => {
    ...
    ///console.log(cards)
    return cards[0].length > 0 || cards[1].length > 0 ? (
      <>
        <div className="row  meter-content mb-10">
          <div className={isMobile ? 'col-lg-12 col12p10' : ' col-lg-12 p0'}>
            <div className="meter-lh-card-container mt-1">
              <h5 className="d-inline-block mb-0">
                ...
                {isMobile && (
                  <div className="runposition">
                    <div className="d-inline-block text-center meter-lh-card ml-1">
                      {userBets?.[0]?.selectionName == 'Low' && (
                        <span style={{ marginLeft: '20px' }}>
                          Run Position :{' '}
                          {parseInt(lastOdds?.['cards']?.C1) -
                            (lastOdds?.['cards']?.C3 == 1 ? 9 : 0) -
                            (parseInt(lastOdds?.['cards']?.C2) +
                              +(lastOdds?.['cards']?.C3 == 1 ? 9 : 0))}
                        </span>
                      )}
                      {userBets &&
              ...
              <div
                ...
                } text-center meter-lh-card ml-1`}
                style={{ display: 'block !important', marginTop: '5px' }}
              >
                {cards &&
                  cards[0].length > 0 &&
                  cards[0].map((Item: any, key: number) => {
                    return (
                      <img
                        key={key}
                        src={`https://dzm0kbaskt4pv.cloudfront.net/v11/static/mobile/img/cards/${Item}.png`}
                        className="cmertercard"
                      />
                    );
                  })}
              </div>
          ...
          <div className={isMobile ? 'col-lg-12 col12p10' : ' col-lg-12 p0'}>
            <div className="meter-lh-card-container mt-1">
              ...
              >
                {cards &&
                  cards[1].length > 0 &&
                  cards[1].map((Item: any, key: number) => {
                    return (
                      <img
                        key={key}
                        src={`https://dzm0kbaskt4pv.cloudfront.net/v11/static/mobile/img/cards/${Item}.png`}
  ...
  return (
    <div className="container ">
      {layoutCard()}
      <div className="row meter-content ">{laybacklayout()}</div>
    </div>
  );
};
