...
const Dragontiger1Day = (props: any) => {
    ...
    return ['Dragon', 'Tiger'].map((ItemNew: any, key: number) => {
      const market1 =
        liveMatchData?.defaultMarkets?.filter(
          (ItemEven: any) => ItemEven.MarketName == `${ItemNew} Even`
        )?.[0]?.Runners?.[0] || {};
      const market2 =
        liveMatchData?.defaultMarkets?.filter(
          (ItemEven: any) => ItemEven.MarketName == `${ItemNew} Odd`
        )?.[0]?.Runners?.[0] || {};
      return (
        <tr key={key}>
          <td className="box-4" style={{ paddingLeft: '10px' }}>
            {ItemNew}
          </td>
          <BackButtonPnl
            key={key}
  ...
  const redblack = () => {
    return ['Dragon', 'Tiger'].map((ItemNew: any, key: number) => {
      const market1 =
        liveMatchData?.defaultMarkets?.filter(
          (ItemEven: any) => ItemEven.MarketName == `${ItemNew} Red`
        )?.[0]?.Runners?.[0] || {};
      const market2 =
        liveMatchData?.defaultMarkets?.filter(
          (ItemEven: any) => ItemEven.MarketName == `${ItemNew} Black`
        )?.[0]?.Runners?.[0] || {};
      return (
        <tr key={key}>
          <td className="box-4" style={{ paddingLeft: '10px' }}>
  ...
  const carditem = () => {
    return ['Dragon', 'Tiger'].map((ItemNew: any, key: number) => {
      const market1 =
        liveMatchData?.defaultMarkets?.filter(
          (ItemEven: any) => ItemEven.MarketName == `${ItemNew} Spade`
        )?.[0]?.Runners?.[0] || {};
      const market2 =
        liveMatchData?.defaultMarkets?.filter(
          (ItemEven: any) => ItemEven.MarketName == `${ItemNew} Heart`
        )?.[0]?.Runners?.[0] || {};
      const market3 =
        liveMatchData?.defaultMarkets?.filter(
          (ItemEven: any) => ItemEven.MarketName == `${ItemNew} Diamond`
        )?.[0]?.Runners?.[0] || {};
      const market4 =
        liveMatchData?.defaultMarkets?.filter(
          (ItemEven: any) => ItemEven.MarketName == `${ItemNew} Club`
        )?.[0]?.Runners?.[0] || {};
      return (
        <tr key={key}>
          <td className="box-2" style={{ paddingLeft: '10px' }}>
            {ItemNew}
          </td>
          <BackButtonPnl
            key={key}
          ...
          <BackButtonPnl
            key={key + 8}
            selectionid={market3.SelectionId}
            lastOdds={lastOdds}
            liveMatchData={liveMatchData}
            clsnamename={'back twentypoker teen-section'}
          />
          <BackButtonPnl
            key={key + 12}
            selectionid={market4.SelectionId}
            lastOdds={lastOdds}
            liveMatchData={liveMatchData}
            clsnamename={'back twentypoker teen-section'}
          />
  ...
  return (
    <div
      className="container "
      id={`${getCurrentMatch?.slug}`}
      style={{ marginTop: '-10px' }}
    >
      <div className="row ">
        <div
          className="col-lg-6 m-b-10 main-market  bg-gray"
          style={{ padding: '0px' }}
        >
        ...
        <div
          ...
          style={{ padding: '20px' }}
        >
          {liveMatchData?.defaultMarkets?.[1]?.Runners.map(
            (ItemNew: any, key: number) => {
